...
BEDRAG_PATRONEN = [
    ...
    (re.compile(r"\b(?:quoted|accepted|commercial)\b"), "commercieel veld uit project.yaml"),
    (re.compile(r"\b(?:marge|offertebedrag|dagtarief|uurtarief)\b", re.I), "prijsterm"),
]

# Bekende uitzonderingen: tekst die op een bedrag lijkt maar het niet is. Elke regel
# draagt een reden. Staat er ooit iets bij zonder reden, dan is dat het signaal dat
# iemand de test heeft stilgelegd in plaats van het lek te repareren.
TOEGESTAAN = [
    # (patroon, reden)
]
...
TOEGESTANE_UITZONDERINGEN = {"company/canon-queue.md"}
    ...
    for rol in BEPERKTE_ROLLEN:
        ...
        for pad in bestanden:
            ...
            for nr, regel in enumerate(tekst.splitlines(), 1):
                if any(toe.search(regel) for toe, _ in TOEGESTAAN):
                    continue
...
def controle_negatief(uitvoer):
    ...
    if met_bedrag and os.path.isdir(admin):
        ...
        for pad in glob.glob(os.path.join(admin, "**", "*.html"), recursive=True):
            with open(pad, encoding="utf-8") as f:
                if any(p.search(f.read()) for p, _ in BEDRAG_PATRONEN):
                    gevonden = True
                    break
        if not gevonden:
            meldingen.append(
                "Let op: er zijn bedragen in de bron, maar ze staan ook niet in de "
                "administrator-uitvoer. Dat is geen lek, maar waarschijnlijk wel een "
                "bug - de zaakvoerders horen ze juist wel te zien.")
    return [], meldingen
...
def main():
    ...
    fouten = harde_fouten + controle_structureel(uitvoer) + controle_uitvoer(uitvoer)
